...
if __name__ == "__main__":
    ...
    for nevent,evt in enumerate(ds.events()):
        ...
        qad_wave = qad_det.get(evt)
        dg2_val = dg2_det.get(evt)
        wave8_val = wave8_det.get(evt)
        hfx_val = hfx_det.get(evt)
        em_val = em_det.get(evt)


        if nevent >= 1000:
            break

    raw_all = wave8_det.raw(evt)
    for raw in raw_all[8:]:
        plt.plot(raw)
    plt.show()

# %% Check waveform offsets qad1

offset_75 = -0.0027809356519596434
...
for nevent,evt in enumerate(ds.events()):
    ...
    qad_list.append(qad_det.raw(evt))
    dg2_val = dg2_det.get(evt)
    wave8_val = wave8_det.get(evt)
    hfx_val = hfx_det.get(evt)
    em_val = em_det.get(evt)
...
for nevent,evt in enumerate(ds.events()):
    ...
    try:
        dg2.append(dg2_det.get(evt).TotalIntensity())
    except:
        dg2.append(-1)
    try:
        fee.append(fee_det.get(evt).f_11_ENRC())
    except:
        fee.append(-1)
    j+=1
