...
if __name__ == "__main__":
    ...
    for i in range(0,radials.shape[0],r_step):
        plt.plot(q[:max_ind],radials[i][:max_ind]) # No normalization
        # plt.plot(q,radials[i] / np.mean(radials[i])) # mean normalization
        # plt.plot(q,radials[i] / dg2[i]) # mean normalization

    mean_rad = radials.mean(0)
    plt.plot(q[:max_ind],mean_rad[:max_ind],'k-',lw=3,label='mean')
    plt.legend()
    plt.xlabel('q')
    plt.ylabel('I(q)')
    plt.title(basename)
    plt.savefig(out_dir + basename+'_radials_noMasks.png',dpi=150)
    # plt.savefig(out_dir + basename+'_radials_dg2Norm.png',dpi=150)
