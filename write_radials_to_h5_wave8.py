...
if __name__ == "__main__":
    ...
    output = f'{conf["hdf5_directory"]}{conf["experiment_id"]}_r{args.run:04d}_{args.detector}_oldMasks.h5'

    # Temp to specify BIGMASK included **CHECK CONFIG.PY
    if args.detector=="epix":
        output = f'{conf["hdf5_directory"]}{conf["experiment_id"]}_r{args.run:04d}_{args.detector}_bigMask.h5'
    ...
    exp_id = "cxil1005322"
    run = str(args.run)
    psi_det = psana.Detector("CXI-DG2-BMMON") # event code
    ...
    # ** CHECK MAKING THE FILE WITH correct_waveform_offsets.py
    ...
    h5.create_dataset('/waveforms', data=waveforms)
    h5.create_dataset('/wftimes', data=wftimes)
    h5.create_dataset('/post_sample_intensity', data=psi)
    h5.close()
